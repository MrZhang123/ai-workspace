...
- For dense financial ratio tables, add a short unordered list directly below the table explaining each metric's meaning, formula, and interpretation. This is required for solvency, liquidity, profitability, capital return, working-capital, and cash-flow-quality tables. For example, below a solvency and liquidity table, explain `流动比率 = 流动资产 / 流动负债`, `速动比率 = (流动资产 - 存货) / 流动负债`, `债务/权益 = 有息债务 / 股东权益`, `利息保障倍数 = EBIT（息税前利润） / 利息费用`, `OCF（经营现金流）/流动负债`, and `净现金 = 现金及可交易证券 - 有息债务`. Include how to read the direction of the metric, such as "higher is usually safer", "below 1 may indicate pressure", or "negative means net debt", and add company-specific caveats when a single ratio can mislead.
